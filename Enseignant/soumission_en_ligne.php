  ...
  <table class="table table-striped table-bordered">
        ...
        if(isset($_POST['filtrer'])){
            ...
            if(!empty($_POST['code']) && empty($_POST['soul'])){
                ...
                //  WHERE  date_debut <= NOW() AND date_fin >= NOW() AND
                //   status = 0 AND code='$code' AND id_ens = (select id_ens from enseignant
                ...
                // $req2 = mysqli_query($conn , $req_sous2);
                    ...
                    $req_sous1 = "SELECT * FROM soumission
                    INNER JOIN matiere USING (id_matiere)
                    WHERE date_debut <= NOW() AND date_fin >= NOW() AND status = 0
                    AND code='$code' AND (id_ens = (SELECT id_ens FROM enseignant WHERE email = '$email')
                    OR id_matiere NOT IN (SELECT id_matiere FROM soumission 
                                            WHERE id_ens = (SELECT id_ens FROM enseignant WHERE email = '$email')))
                ...
                WHERE date_debut <= NOW() AND date_fin >= NOW() AND status = 0
                AND code='$code' AND (id_ens != (SELECT id_ens FROM enseignant WHERE email = '$email')
                OR id_matiere IN (SELECT id_matiere FROM soumission 
                                WHERE id_ens = (SELECT id_ens FROM enseignant WHERE email = '$email')))
                ORDER BY date_fin DESC";

            $req2 = mysqli_query($conn, $req_sous1);
            }
            ...
            elseif(!empty($_POST['code']) && !empty($_POST['soul'])){
                ...
                $req_sous1 =  "SELECT * FROM soumission
                 inner join matiere using(id_matiere)  
                 WHERE  date_debut <= NOW() AND date_fin >= NOW() AND
                  status = 0  AND code='$code' AND 
                  id_sous in (SELECT id_sous FROM $type )  AND
                   id_ens = (select id_ens from enseignant 
                   where email = '$email')  ORDER BY date_fin DESC ";
                $req1 = mysqli_query($conn , $req_sous1);
                $req_sous2 =  "SELECT * FROM soumission
...
if (isset($_SESSION['cloture_reussi']) && $_SESSION['cloture_reussi'] === true) {
  echo "<script>
  Swal.fire({
...
if (isset($_SESSION['archive_reussi_ligne']) && $_SESSION['archive_reussi_ligne'] === true) {
  echo "<script>
  Swal.fire({
